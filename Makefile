...
.PHONY: install lint test coverage build-publish-pypi run run-lambda invoke-lambda-args terraform-apply terraform-plan all
...
build-push: build login create-repo push
